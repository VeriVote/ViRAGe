...
(* An Electoral Module is decrementing, iff it rejects at least one alternative,
   whenever possible (|A|>1). *)
definition decrementing :: "'a Electoral_module \<Rightarrow> bool" where
  "decrementing m \<equiv> electoral_module m \<and> (
    \<forall> A p . finite_profile A p \<longrightarrow> (
       card A > 1 \<longrightarrow> card (reject m A p) \<ge> 1
  ))"

(* An Electoral Module is defer_deciding, iff it choses exactly 1
   alternativ to defer and rejects any other alternative.
   Note that rejects n-1 m can be ommited due to partition property *)
definition defer_deciding :: "'a Electoral_module \<Rightarrow> bool" where
  "defer_deciding m \<equiv> electoral_module m \<and>
    non_electing m \<and> defers 1 m"


(*
(* An Electoral Module m is rejecting, iff at least one alternative gets rejected when possible *)
...
(* Falsche Definition, wähle n > card A, schon ist es immer true *)
(* An Electoral Module m is eliminating, iff the following holds *)
(* If there is at least one alternative that can be rejected, at least one alternative gets rejected *)
...
lemma upper_bounds_for_elect_reject_defer_size:
  assumes em : "electoral_module m" and
         profile : "finite_profile A p"
       shows "card (elect m A p) \<le> card A \<and> card (reject m A p) \<le> card A \<and> card (defer m A p) \<le> card A " by (simp add: card_mono defer_from_input elect_from_input em profile reject_from_input)
