...
ML \<open>
    ...
    val y = filter (fn x => (String.isPrefix ("Sequential_Composition.sequential_composition") (snd (fst (x))))) 
        (Defs.all_specifications_of (Theory.defs_of @{theory}))
    val z = filter (fn x => String.isPrefix ("Sequential_Composition.sequential_composition_def") (#description x)) (map (fn x => (hd (snd x))) y)
...
ML \<open> val facts = Global_Theory.facts_of @{theory} 
     val filtered = filter (fn x => true) []\<close>

ML_val \<open>val seq_consts = filter (fn x => (String.isPrefix (Context.theory_name @{theory}) (snd (fst (x))))) 
    (Defs.all_specifications_of (Theory.defs_of @{theory}))
    val names = map (fn x => (#description (hd (snd(x))))) seq_consts
    val seq_defs = filter (String.isSuffix ("_def")) names 
    val strip_fun = fn x => String.substring ((x),0,((String.size x)-4))
    val seq_defs_stripped = map strip_fun seq_defs

    fun member_of (item, list) = List.exists (fn x => x = item) list
    val filter_fun = fn x => member_of (x, seq_defs_stripped)

    val constants = #constants (Consts.dest (Sign.consts_of @{theory}))
    val filtered_constants = filter (filter_fun) (map fst constants)

    val reduce_noise = fn x => not ((String.isSuffix "_graph" x) 
                                        orelse (String.isSuffix "_sumC" x) 
                                        orelse (String.isSuffix "_rel" x)) 

    val actual_constants = filter reduce_noise filtered_constants

    val constants = #constants (Consts.dest (Sign.consts_of @{theory}))
    val filter_fun2 = fn x => member_of (fst x, actual_constants)
    val sign = map (fn x => (fst (snd x))) (filter filter_fun2 constants)



    val x = hd sign

    val x_string = Syntax.string_of_typ_global @{theory} x

    val x_pretty = Syntax.pretty_typ_global @{theory} x

    val x_pretty_string = Pretty.string_of x_pretty

    val y = Logic.mk_type x

    val n = Term.size_of_typ x

    val pm = fn Basic_Term.Type x => snd x
    ...
    val z = Term.is_Bound y



val a = fn thy => ((map (fn x => (fst x)) (#constants (Consts.dest (Sign.consts_of thy)))),map (fn x => let fun typ_to_string (x: Basic_Term.typ): string = case x of
Type x => "(" ^ (fst x) ^ String.concat (map (typ_to_string) (snd x)) ^ ")"
| _ => "(?'a)" in typ_to_string x end)(map (fn x => (fst (snd x))) (#constants (Consts.dest (Sign.consts_of thy)))))
...
ML \<open>
    ...
    val sign2 =rev  ( map (fn x => (fst  x)) constants ) \<close>


ML_val \<open>
val thms = map (fn x => fst (snd x)) (Global_Theory.dest_thm_names @{theory});
val thm = Global_Theory.get_thm @{theory} (hd (rev thms));
(Syntax.string_of_term_global \<^theory>\<open>Main\<close> (Thm.prop_of thm)) |> writeln
\<close> 
