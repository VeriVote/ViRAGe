...
public class PrologProofTest {
  ...
  public void testSmcMonotone() {
    String input = "subgoal(monotone(sequential_composition(loop_composition("
        ...
        + "_module(2), sequential_composition(downgrade(plurality_module), pass_module(1))),"
        + " drop_module(2), max_aggregator), defer_"
        ...
        + "al_composition(pass_module(2), sequential_composition(downgrade(plurality_module),"
        + " pass_module(1))), drop_module(2), max_"
    ...
    String reference = "monotone(sequential_composition(loop_composition(parallel_composition("
        ...
        + "pass_module(2),sequential_composition(downgrade(plurality_module),pass_module(1))),"
        + "drop_module(2),max_aggregator),defer_eq_condition(1)))\n"
        + "		defer_lift_invariant(parallel_composition(sequential_composition("
        + "pass_module(2),sequential_composition(downgrade(plurality_module),pass_module(1))),"
        + "drop_module(2),max_aggregator))\n"
        + "			disjoint_compatible(sequential_composition(pass_module(2),sequential_composition("
        + "downgrade(plurality_module),pass_module(1))),drop_module(2))\n"
        ...
        + "				defer_lift_invariant(sequential_composition(downgrade(plurality_module),"
        + "pass_module(1)))\n"
        ...
        + "	non_electing(loop_composition(parallel_composition(sequential_composition("
        + "pass_module(2),"
        + "sequential_composition(downgrade(plurality_module),pass_module(1))),drop_module(2),"
        + "max_aggregator),defer_eq_condition(1)))\n"
        + "		non_electing(parallel_composition(sequential_composition(pass_module(2),"
        + "sequential_composition(downgrade(plurality_module),pass_module(1))),drop_module(2),"
        + "max_aggregator))\n"
        + "			non_electing(sequential_composition(pass_module(2),sequential_composition("
        + "downgrade(plurality_module),pass_module(1))))\n"
        + "				non_electing(pass_module(2))\n"
        + "				non_electing(sequential_composition(downgrade(plurality_module),"
        + "pass_module(1)))\n"
        + "					non_electing(downgrade(plurality_module))\n"
        + "					non_electing(pass_module(1))\n" + "			non_electing(drop_module(2))\n"
        + "			conservative(max_aggregator)\n"
        + "	defers(loop_composition(parallel_composition(sequential_composition(pass_module(2),"
        + "sequential_composition(downgrade(plurality_module),pass_module(1))),drop_module(2),"
