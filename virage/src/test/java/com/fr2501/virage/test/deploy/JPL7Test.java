		...
		try {
			...
			if(!version.equals(referenceVersion)) {
				logger.warn("You are using JPL version " + version + ". "
						+ "ViRAGe was developed and tested using JPL version "
						+ referenceVersion + ". If the program seems to behave "
						+ "weirdly when running Prolog commands, consider "
						+ "switching to that version.");
			}

		} catch (ClassNotFoundException e) {
			logger.error("JPL7 could not be located. Please "
					+ "check whether it is set up correctly, "
					+ "according to the ViRAGe ReadMe and the "
					+ "instructions given on jpl7.org.");
			fail();
		}
	}
}
