 ...
 * (@see <a href="file:../../../../../../../../../src/main/resources/">resources</a>),
 * this class is able to make the Prolog search process more transparent, exposing goals and
 * subgoals for every proof step.
 *
 * @author VeriVote
 */
public final class PrologProof {
    ...
    public static PrologProof createProofFromString(final String string) {
        ...
        for (int i = 0; i < subgoals.length; i++) {
            ...
            // A bit of sanitation
            subgoals[i] = subgoals[i].replace(MORE_GOALS_ON_LEVEL, StringUtils.EMPTY);
            ...
            if (closesBranch[i]) {
                ...
                final String regex = StringUtils.COMMA + BRANCH_CLOSE + StringUtils.PERIOD + "*";
                subgoals[i] = subgoals[i].replaceAll(regex, StringUtils.EMPTY);
            }
            // More sanitation
            if (subgoals[i].startsWith(StringUtils.OPENING_PARENTHESIS)) {
                subgoals[i] = subgoals[i].substring(1, subgoals[i].length());
            }
            if (subgoals[i].endsWith(StringUtils.COMMA +  StringUtils.OPENING_PARENTHESIS)) {
                subgoals[i] = subgoals[i].substring(0, subgoals[i].length() - 2);
            }
            if (subgoals[i].endsWith(StringUtils.COMMA)) {
