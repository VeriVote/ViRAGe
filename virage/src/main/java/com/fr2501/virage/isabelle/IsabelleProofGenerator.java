...
public class IsabelleProofGenerator {
			...
			if(subgoal.getAllCompositionRules().size() == 1) {
				...
				if(rule.getOrigin().equals("generated")) {					
					...
					for(ComponentType child: p.getParameters()) {
						// Only parameters defined within the module definition can be referenced, so if any others
						// occur, we skip.
						...
						newAssumptions += "\"";
						System.out.println(newAssumptions);
	...
	private String replaceVariables(String theoremName, String goal, 
			String proofSteps, String subgoalIds, String assumptions) {
